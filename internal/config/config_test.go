...
func TestValidate_RejectsEachInvariant(t *testing.T) {
	cases := []struct {
		...
		{"seasonality interval cv < 0", func(c *Config) { c.SeasonalityMaxIntervalCV = -0.1 }, "seasonality max interval cv"},
		{"align tolerance < 0", func(c *Config) { c.AlignTolerance = -time.Second }, "align tolerance"},
		...
		{"correlation min points < 3", func(c *Config) { c.CorrelationMinPoints = 2 }, "correlation min points"},
		...
		{"divergence r2 > 1", func(c *Config) { c.DivergenceMinR2 = 1.5 }, "divergence min r2"},
		{"convergence r2 < 0", func(c *Config) { c.ConvergenceMinR2 = -0.1 }, "convergence min r2"},
